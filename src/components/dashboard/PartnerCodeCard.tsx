...
const PartnerCodeCard = ({
    ...
    const createOrGetShortUrl = async () => {
      ...
      try {
        ...
        const {
          data: existingUrl,
          error: existingError
        } = await supabase.from('short_urls').select('short_code').eq('target_url', `https://t.me/+jW5YO_GG7Z05NmI9`).single();
        if (existingUrl) {
        ...
        if (!existingError || existingError.code === 'PGRST116') {
          ...
          const {
            ...
            user_id: profile.id,
            target_url: `https://t.me/+jW5YO_GG7Z05NmI9`,
  ...
  const shareToTwitter = () => {
    const shareText = `Join the Wealth Elites Partner Program and earn $1,000 per 10,000 clicks. Use my link below:`;
